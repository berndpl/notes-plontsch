// Breathing Variant 01 - Original Baseline
// Standard configuration: 6s cycle, 24 nodes, standard easing, random speed variation
(function() {
    'use strict';
    
    function initBreathing() {
        const canvas = document.getElementById('breathing-variant-01-canvas');
        if (!canvas) {
            console.warn('Breathing Variant 01: Canvas element not found');
            return;
        }
        
        const ctx = canvas.getContext('2d');
        if (!ctx) {
            console.error('Breathing Variant 01: Could not get 2D context');
            return;
        }
        ...
        function getPhase(t) {
            ...
            if (cycleNumber !== currentCycle) {
                ...
                for (let i = 0; i < numExplorationNodes; i++) {
                    const offset = (Math.random() * 2 - 1) * nodeSpeedVariation;
        ...
        // Initialize with default timing (will be updated by preset)
