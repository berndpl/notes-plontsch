// Breathing Variant 09 - Wave Movement
// Nodes expand sequentially in a wave pattern around the circle
(function() {
    'use strict';
    
    function initBreathing() {
        const canvas = document.getElementById('breathing-variant-09-canvas');
        if (!canvas) {
            console.warn('Breathing Variant 09: Canvas element not found');
            return;
        }
        
        const ctx = canvas.getContext('2d');
        if (!ctx) {
            console.error('Breathing Variant 09: Could not get 2D context');
            return;
        }
        ...
        // Wave configuration
        const waveDelay = 0.06; // Maximum delay for wave to complete (as fraction of expandDuration)
        ...
        // Get node-specific phase offset for wave movement
        function getNodeWaveOffset(nodeIndex) {
            const nodePosition = nodeIndex / numExplorationNodes;
            return nodePosition * waveDelay;
        }
        
        function getNodeRadius(phase, nodeIndex) {
            const waveOffset = getNodeWaveOffset(nodeIndex);
            let adjustedPhase = phase;
            
            if (phase < expandEnd) {
                // Wave in: nodes expand sequentially
                adjustedPhase = phase - waveOffset;
                adjustedPhase = Math.max(0, Math.min(expandEnd, adjustedPhase));
            } else if (phase < expandRestEnd) {
                // Rest phase: allow nodes to catch up smoothly
                const remainingExpansion = phase - expandEnd;
                const catchUpPhase = expandEnd - waveOffset + remainingExpansion;
                ...
                // Collapse: all nodes collapse together (synchronized, no wave)
                adjustedPhase = phase;
            } else {
