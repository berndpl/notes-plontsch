// Breathing Variant 08 - Synchronized Expansion
// All nodes expand at exactly the same speed - no variation, fully synchronized
(function() {
    'use strict';
    
    function initBreathing() {
        const canvas = document.getElementById('breathing-variant-08-canvas');
        if (!canvas) {
            console.warn('Breathing Variant 08: Canvas element not found');
            return;
        }
        
        const ctx = canvas.getContext('2d');
        if (!ctx) {
            console.error('Breathing Variant 08: Could not get 2D context');
            return;
        }
        ...
        // All nodes use the same radius - no variation
        function getNodeRadius(phase, nodeIndex) {
            // No speed variation - all nodes expand at exactly the same rate
            return getExplorationRadius(phase);
        }
